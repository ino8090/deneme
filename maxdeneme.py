...
M3U_URL = os.getenv("M3U_URL", "https://raw.githubusercontent.com/ino8090/0101/refs/heads/main/yerli.m3u")
LOGO_URL = os.getenv("LOGO_URL", "https://raw.githubusercontent.com/ino8090/0101/refs/heads/main/1787745128505.png")

# Opaklık Ayarları (0.0 ile 1.0 arası: 0.8 = %80 görünürlük / %20 saydamlık)
LOGO_OPACITY = 0.8
TEXT_OPACITY = 0.8
...
def start_m3u_stream():
    ...
    while True:
        ...
        # Logo İşleme (Sağ Üst Köşe + Opaklık)
        if has_logo:
            logo_idx = next_input_index
            overlay_inputs.extend(['-i', 'logo.png'])
            next_input_index += 1
            filter_steps.append(f'[{logo_idx}:v]scale=-2:80,format=rgba,colorchannelmixer=aa={LOGO_OPACITY}[logo]')
            filter_steps.append(f'{last_stream}[logo]overlay=main_w-overlay_w-50:50[v_logo]')
            last_stream = '[v_logo]'

        # Sol Alt Köşeye Kalın Yazı ile Film Adı Ekleme (Opaklık Ayarlı)
        # FFmpeg drawtext için özel karakter kaçışları (escape)
        clean_title = film_title.replace("'", "").replace(":", "\\:").replace("%", "\\%")
        filter_steps.append(
            f"{last_stream}drawtext=text='{clean_title}':x=50:y=h-th-50:"
            f"fontsize=36:fontcolor=white@{TEXT_OPACITY}:font='Arial-Bold':"
            f"shadowcolor=black@{TEXT_OPACITY}:shadowx=2:shadowy=2[v_text]"
        )
        last_stream = '[v_text]'
